...
const readKeywords = () => {
    if (!fs.existsSync(KEYWORDS_FILE)) {
        throw new Error("File keywords.txt not found!");
    }
...
const startProcess = async (processIndex, keywords) => {
    while (true) {
        ...
        console.log(`🔄 Process ${processIndex} waiting ${INTERVAL / 1000} seconds...`);
...
rl.question("Enter the number of processes you want: ", (answer) => {
    const processCount = parseInt(answer, 10);
    if (isNaN(processCount) || processCount <= 0) {
        console.log("❌ Invalid number of processes!");
        process.exit(1);
    }
    console.log(`🚀 Running ${processCount} processes...`);
